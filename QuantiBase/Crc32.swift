...
public func ==(lhs: CRC32, rhs: CRC32) -> Bool {
    return lhs.initialized == rhs.initialized && lhs.crc == rhs.crc
}

public final class CRC32: Hashable {
    fileprivate var initialized = false
    public fileprivate(set) var crc: UInt32 = 0
    ...
    public func run(buffer: UnsafePointer<UInt8>, length: Int) {
        crc = crc32(crc, buffer: buffer, length: length)
        initialized = true
    }
    
    public func run(data: Data) {
        crc = crc32(crc, data: data)
        initialized = true
    }
}
